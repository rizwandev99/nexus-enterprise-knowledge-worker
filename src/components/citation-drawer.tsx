  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <aside
          className="w-screen max-w-md sm:max-w-lg lg:max-w-xl flex flex-col h-full overflow-hidden text-slate-100 shadow-2xl transition-all duration-300 ease-out animate-in slide-in-from-right duration-300 bg-[#181c26] border-l border-slate-800/80"
        >
          {/* Top Header */}
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800/80 bg-[#1e2330]/80 shrink-0">
            <div className="flex items-center gap-2.5">
              <span className="px-2.5 py-1 rounded-xl text-xs font-mono font-bold bg-slate-800 border border-slate-700/80 text-white shadow-sm">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800/80 border border-transparent hover:border-slate-700/60 transition-colors cursor-pointer"
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {isLoading ? (
              ...
              <>
                ...
                <div className="p-4 rounded-2xl bg-[#1e2330]/70 border border-slate-800/80 shadow-md space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-mono font-bold bg-slate-800 border border-slate-700 text-white shadow-sm">
                        {matchPct}%
                      </span>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2.5 text-xs font-mono">
                    <div className="p-3 rounded-xl bg-[#1e2330]/60 border border-slate-800/80 flex flex-col shadow-sm">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1 bg-[#1e2330] p-1 rounded-xl border border-slate-800/80 text-xs font-mono">
                      <button
                        type="button"
                        onClick={() => setActiveTab("passage")}
                        className={`px-3 py-1 rounded-lg transition-all cursor-pointer ${
                          activeTab === "passage"
                            ? "bg-slate-700/90 text-white font-semibold shadow-sm"
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-mono bg-[#1e2330] hover:bg-slate-800 border border-slate-800/80 hover:border-slate-700 text-slate-300 hover:text-white transition-all cursor-pointer active:scale-95 shadow-sm"
                      title="1-Click Copy"
                    >
                  ...
                  <div className="rounded-2xl p-4 bg-[#1e2330]/60 border border-slate-800/80 font-mono text-xs leading-relaxed text-slate-200 whitespace-pre-wrap max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700/50 shadow-inner">
                    {activeTab === "passage"
          ...
          {/* Footer */}
          <div className="p-4 border-t border-slate-800/80 bg-[#1e2330]/80 shrink-0 flex items-center justify-between">
